...
- (void)viewDidLoad
{
    ...
    pv.dataSource = self;
    if (event_id > 0) {
        [pv selectRow:(event_id-1) inComponent:0 animated:YES];
    } else {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    events = [EventPeer getEventsByCategoryId:category_id];
    
    [names removeAllObjects];
    [ids removeAllObjects];
    for (Event *e in events) {
        [names addObject:e.name];
        [ids addObject:[NSNumber numberWithInt:e.Id]];
    }
...
- (void)finish:(UIButton *)btn
{
    ...
    if ([_delegate respondsToSelector:@selector(selectEvent:name:)]) {
        [_delegate selectEvent:[[ids objectAtIndex:[pv selectedRowInComponent:0]] intValue] name:[names objectAtIndex:[pv selectedRowInComponent:0]]];
    }
}
